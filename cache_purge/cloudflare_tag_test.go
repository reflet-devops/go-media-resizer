...
func Test_cloudflareTag_Purge(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockFn: func(mockClient *mockTypes.MockClient) {
				mockClient.EXPECT().DoTimeout(gomock.Cond(func(req *fasthttp.Request) bool {
					if !bytes.Equal(req.Body(), []byte(`{"tags":["source_path_hash_df781cc81757931f","source_path_hash_c6047f34708c36213118502d98d05466"]}`)) {
						return false
					}
					return true
		...
		},
		{
			...
			mockFn: func(mockClient *mockTypes.MockClient) {
				mockClient.EXPECT().DoTimeout(gomock.Cond(func(req *fasthttp.Request) bool {
					if !bytes.Equal(req.Body(), []byte(`{"tags":["source_path_hash_0d4ca1fa31a01680","source_path_hash_b7c209b0abcb1d2bc96952a8a512ee12"]}`)) {
						return false
					}
					return true
				}), gomock.Any(), gomock.Any()).DoAndReturn(
